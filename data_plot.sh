...
build/src/pst-batch-training modified_sequences/"original_${outputfile}.fna" --min-count $min_count --max-depth $depth --threshold $threshold -o "	original_${outputfile}_trees.h5";
echo "Calculating VLMC distances";
build/src/calculate-distances -p "original_${outputfile}_trees.h5" -y "${outputfile}_trees.h5" -s "${outputfile}_dists.h5";
